...
%lb = [1e-10 0 0 0];
%ub = [1 1e10 10 viral_data(1)];
lb = [0 1e3 0 0];
ub = [1e-2 1e10 100 viral_data(1)];
%ub = [1e1 1e10 30 viral_data(1)];
...
[xmultinonlin,errormultinonlin,exitflag,output] = run(ms,problem,2000);
...
[time, U, V] = modelsimulator_ODE_case1_v3(p,tf,[initialconds]);
...
function  val = residualsfunction(param)
   ...
   % control_U = interp1(time,U,cell_time');
   ...
   % cell_viab = Usol./control_U'*100;

        Vsol = interp1(time,V,viral_time);
        val_cell_viab = 0;%[cell_viab'- cell_data]./max(cell_data);

        val_viral_proj = [Vsol - viral_data]/max(viral_data);%./p.alpha];%/max(viral_data);

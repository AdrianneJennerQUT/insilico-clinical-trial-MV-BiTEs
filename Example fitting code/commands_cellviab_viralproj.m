...
%parameters
p.r = 0.004;%0.004;%log(2)/24;

p.beta = 1;
p.eta = 100000;
p.d_V = 0.1;
p.alpha = 0.2;%viral_data(1)*0.02;
p.h = 1;

tf = 96;

initialconds = [1e5 1e2];
...
initialconds = [1e5 0.5];

[time, U, V,xmultinonlin,CI] = fit_ODE_model(p,tf,initialconds,cell_data,viral_data,cell_time,viral_time);
...
plot(time,V*p.alpha,'Color',Virus_colour,'LineWidth',2.5)
set(gca,'yscale','log')
xlabel('Time (hours)')
ylabel('Viral progeny (log)')
set(gca,'FontSize',18)
...
%%
    ...
    p.beta = 270.8/24;%xmultinonlin(1);
    p.eta = 9.04*1e5;%xmultinonlin(2);
    p.d_V = 23.11/24;%xmultinonlin(3);
    p.r = 0.004;%xmultinonlin(4);
    p.alpha = 100.56;%xmultinonlin(5);

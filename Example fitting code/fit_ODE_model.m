...
param_guess = [p.beta, p.eta, p.d_V, p.r, p.alpha];

lb = [1e-5 0 0 0 0];
ub = [100 1e7 10 1 viral_data(1)];
...
[xmultinonlin_2,resnorm,residual,exitflag,fit_out,lambda,jacobian] = lsqnonlin(@residualsfunction,param_guess,[lb],[ub]);%,options);%fmincon(@residualsfunction, parameter_guess, [0 0 0 0], [Inf Inf Inf Inf], options);  % can use fmincon/ lsqnonlin
...
p.r =  xmultinonlin(4);%log(2)/24;%0.004;%
p.alpha =xmultinonlin(5);
...
function  val = residualsfunction(param)
    ...
    [time, U, V] = modelsimulator_ODE_case1(p,tf,[initialconds(1),0]);
    control = interp1(time,U,cell_time);

function [time,U,D,V,B,T] = fullmodsim(p,initial_conditions,tf,dose_V,dose_B)


[sol_1] = ode45(@(t,z)mod(t,z,p),[0 1],initial_conditions);
[sol_2] = ode45(@(t,z)mod(t,z,p),[1 2],sol_1.y(:,end)'+[0 0 p.V0*dose_V p.B0*dose_B 0]);
[sol_3] = ode45(@(t,z)mod(t,z,p),[2 3],sol_2.y(:,end)'+[0 0 p.V0*dose_V p.B0*dose_B 0]);
[sol_4] = ode45(@(t,z)mod(t,z,p),[3 4],sol_3.y(:,end)'+[0 0 p.V0*dose_V p.B0*dose_B 0]);
[sol_5] = ode45(@(t,z)mod(t,z,p),[4 tf],sol_4.y(:,end)'+[0 0 p.V0*dose_V p.B0*dose_B 0]);
